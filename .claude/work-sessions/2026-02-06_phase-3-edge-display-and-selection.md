...
- [ ] Add `Edge` interface to `src/types/index.ts` — fields: `from` (string), `to` (string), `value` (number)
- [ ] Add `loadEdges()` to `src/utils/csvLoader.ts` — import `edges.csv?raw`, define `EdgeRow` interface, parse and map to `Edge[]`
- [ ] Verify edges load without errors (console check or dev tools)

## Commit 2: Edge component and SVG rendering

- [ ] Create `src/components/Edge.tsx` — accepts `x1`, `y1`, `x2`, `y2`, `value`, `selected`, `edgeKey`, `onClick` props
...
- [ ] Add SVG layer to `MapCanvas` — full-size `<svg>` element positioned behind the node divs
- [ ] Load edges in `MapCanvas` (call `loadEdges()` at module scope, same pattern as nodes)
- [ ] Build a position lookup: map canvasNode town names to `{ x, y }` for resolving edge endpoints
- [ ] Render `Edge` components inside the SVG layer, passing resolved coordinates
- [ ] Confirm edges render correctly between the right nodes (visual check)

## Commit 3: Selection state and interactivity

- [ ] Add `selectedEdgeKeys` state to `App` (`Set<string>`, per ADR-001)
- [ ] Add `handleToggleEdge` handler in `App` — toggles a key in/out of the set
- [ ] Pass `selectedEdgeKeys` and `onToggleEdge` as props to `MapCanvas`
- [ ] Update `MapCanvas` props interface to accept selection state and callback
- [ ] Wire each `Edge` component's `onClick` to call `onToggleEdge` with the edge's key (`${from}-${to}`)
- [ ] Pass `selected` boolean to each `Edge` (derived from `selectedEdgeKeys.has(key)`)
- [ ] Style selected edges distinctly — thicker stroke, accent colour
- [ ] Verify toggle works: click edge to select, click again to deselect (visual check)
